#!/usr/bin/env python3
"""
اسکریپت راه‌اندازی ربات ستاره داوود
Script to run Magen David bot
"""
...
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - Magen David - %(levelname)s - %(message)s',
    handlers=[
        logging.FileHandler('logs/magen_david_standalone.log'),
...
async def main():
    ...
    try:
        ...
        # import کردن ربات
        from bots.magen_david import MagenDavidBot
        from config import BOT_TOKENS
        
        # بررسی توکن
        token = BOT_TOKENS.get("magen_david")
        if not token:
            logger.error("❌ توکن ربات ستاره داوود یافت نشد!")
            return
        
        logger.info("🚀 راه‌اندازی ربات ستاره داوود...")
