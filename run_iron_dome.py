#!/usr/bin/env python3
"""
اسکریپت راه‌اندازی ربات گنبد آهنین اسرائیل
Script to run Iron Dome bot
"""
...
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - Iron Dome - %(levelname)s - %(message)s',
    handlers=[
        logging.FileHandler('logs/iron_dome_standalone.log'),
...
async def main():
    ...
    try:
        ...
        # import کردن ربات
        from bots.iron_dome import IronDomeBot
        from config import BOT_TOKENS
        
        # بررسی توکن
        token = BOT_TOKENS.get("iron_dome")
        if not token:
            logger.error("❌ توکن ربات گنبد آهنین یافت نشد!")
            return
        
        logger.info("🛡️ راه‌اندازی ربات گنبد آهنین اسرائیل...")
...
if __name__ == "__main__":
    print("🛡️ راه‌اندازی ربات گنبد آهنین اسرائیل...")
