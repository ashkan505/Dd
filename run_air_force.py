#!/usr/bin/env python3
"""
اسکریپت راه‌اندازی ربات نیروی هوایی اسرائیل
Script to run Air Force bot
"""
...
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - Air Force - %(levelname)s - %(message)s',
    handlers=[
        logging.FileHandler('logs/air_force_standalone.log'),
...
async def main():
    ...
    try:
        ...
        # import کردن ربات
        from bots.air_force import AirForceBot
        from config import BOT_TOKENS
        
        # بررسی توکن
        token = BOT_TOKENS.get("air_force")
        if not token:
            logger.error("❌ توکن ربات نیروی هوایی یافت نشد!")
            return
        
        logger.info("🚀 راه‌اندازی ربات نیروی هوایی اسرائیل...")
